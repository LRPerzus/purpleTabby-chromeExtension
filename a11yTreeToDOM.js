      ...
      xpaths.forEach((path) => {
        ...
        try {
          ...
          while (
            ...
            indexOfChild <= childrenOfElement.length - 1
          ) {
            ...
            if (
              indexOfChild === 0 &&
              childrenOfElement.length - 1 < childIndex
            ) {
              console.log(
                'The tree has skipped a div(this can happen sometimes)'
              )
              missingDiv = child
            }
            ...
            console.log('end?', end)
            console.log(
              'indexOfChild >= changesChildIndex',
              indexOfChild >= changesChildIndex
            )
            ...
            if (
              ...
              if (child.getAttribute('rel').toLowerCase() === 'stylesheet') {
                // Some links are stylesheets
                changesChildIndex++
              }
            } else if (
              (visibale && child.shadowRoot) ||
              window.getComputedStyle(child).display === 'none'
            ) {
              console.log('Visiable or ShadowRoot')
              changesChildIndex++
            } else if (
              xpathsTraverseIndex + 1 !== xpaths.length &&
              ...
              // If it passes all conditions and is a possible element
              if (changesChildIndex !== childIndex) {
                ...
                console.log(
                  'hey indexOfChild >= changesChildIndex:',
                  indexOfChild >= changesChildIndex
                )
                end = true
              }
          ...
          if (element.shadowRoot) {
            ...
            if (changesChildIndex === indexOfChild) {
              if (changesChildIndex > childrenOfElement.length - 1) {
                // Goes out of the index length
                changesChildIndex = childIndex
              }
            }
