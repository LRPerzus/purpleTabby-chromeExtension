...
import {
  getFrameTree,
  processFrameTrees,
  settingAttributeNode,
} from './background functions/frameTreesFuncs.js'
import { collectDOMNodes } from './background functions/domTreeFunc.js'
import {
  setAttributeValue,
  areScansFinished,
} from './background functions/common.js'
import {
  storeDataForTab,
  clearLocalStorage,
  getFromLocal,
} from './background functions/localStorageFunc.js'
  ...
  if (request.type === 'PlUGIN_CLICKED') {
    ...
    try {
      ...
      const scriptChecks = [
        {
          name: 'content.js',
          message: 'CHECK_CONTENT_JS',
          status: 'CONTENT_READY',
        },
        {
          name: 'scanningProcess.js',
          message: 'CHECK_SCANNING_PROCESS_JS',
          status: 'SCANNING_PROCESS_READY',
        },
        {
          name: 'a11yTreeListeners.js',
          message: 'CHECK_A11Y_TREE_LISTENERS_JS',
          status: 'A11Y_LISTENERS_READY',
        },
        {
          name: 'getClickableElementsListeners.js',
          message: 'CHECK_CLICKABLE_ELEMENTS_LISTENERS_JS',
          status: 'CLICKABLE_ELEMENTS_READY',
        },
        {
          name: 'overlayListeners.js',
          message: 'CHECK_OVERLAY_LISTENERS_JS',
          status: 'OVERLAY_LISTENERS_READY',
        },
        {
          name: 'getClickableItems.js',
          message: 'CHECK_GETCLICKABLE_JS',
          status: 'GET_GETCLICKABL_READY',
        },
        {
          name: 'attachMutationObserver.js',
          message: 'CHECK_MUTATIONOBSERVER_JS',
          status: 'MUTATIONOBSERVER_READY',
        },
        {
          name: 'screenshotElement.js',
          message: 'CHECK_SCREENSHOTELEMENT_JS',
          status: 'SCREENSHOTELEMENT_READY',
        },
      ]
      ...
      chrome.runtime.sendMessage(
        { type: 'PLUGIN_READY', tabId: tabId },
        (response) => {
          if (chrome.runtime.lastError) {
            console.error(
              "Failed to send 'PLUGIN_READY' message:",
    ...
    const missingXpath = await getFromLocal(
      request.tabId,
      'missingXpath',
      false,
      request.siteUrl
    ...
    console.log(
      'Overlay created, preparing to request AX tree and clickableElements.'
    )
    const tabId = request.tabId
    const siteUrl = request.siteUrl
    console.log('OVERLAY_CREATED tabId', tabId)
    const missingXpaths = await getFromLocal(
      tabId,
      'missingXpath',
      false,
      siteUrl
    ...
    if (
      missingXpaths !== undefined ||
      ((await isDebuggerAttached(request.tabId)) === undefined &&
        !debuggerAttached[tabId] &&
        debuggerAttached[tabId] !== true)
    ) {
      chrome.runtime.sendMessage({
        type: 'UPDATE_OVERLAY',
        data: missingXpaths,
        settings: settings[request.tabId],
        tabId: request.tabId,
      })
    } else {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      ...
      if (tabId) {
        ...
        if (!(debuggerAttached[tabId] && debuggerAttached[tabId] === true)) {
          ...
          scanningQueueDictionary[tabId].redo = true
          console.log(
            'UPDATED scanningQueueDictionary[tabId].redo',
            scanningQueueDictionary[tabId].redo
          )
          ...
          if (request.tabId) {
            if (request.tabId === tabId) {
              ...
              chrome.tabs.sendMessage(tabId, {
                type: 'Can_find_clickable',
                tabId: tabId,
              })
              sendResponse({ success: true })
            } else {
              console.log(
                `The active tab has moved (tabId Query ${tabId} | request.tabId ${request.tabId})`
              )
    ...
    try {
      const tabId = request.tabId
      console.log(
        'When GET_AX_TREE is triggered debugger is:',
        await isDebuggerAttached(tabId)
      )
      ...
      if (debuggerAttached[tabId]) {
        ...
        const [domResults, frameTreeResults] = await Promise.all([
          ...
          frameTreePromise.then((frameTree) =>
            processFrameTrees(tabId, frameTree)
          ),
        ]).catch((error) => {
    ...
    console.log(
      'A11yTree_DOM_XPATHS_DONE request.data.tabId',
      request.data.tabId
    )
    storeDataForTab(
      request.data.tabId,
      request.data.foundElements,
      'foundElements',
      noClicks
    )
    ...
    // previous errors data
    const previousMissingXpath = await getFromLocal(
      tabId,
      'missingXpath',
      false,
      request.siteurl
    ...
    if (previousMissingXpath) {
      console.log('THERE WAS PRIVUOUS DATA')
      // cause each is its own dict kinda
      ;({ mergedFramesDict, mergedMissingList } = mergeDictionaries(
        request.data.framesDict,
        previousMissingXpath.framesDict,
        mergedMissingList
      ))
    } else {
      ;(mergedFramesDict = request.data.framesDict),
    ...
    console.log(
      'MISSING_FOUND scanningQueueDictionary[tabId].redo ',
      scanningQueueDictionary[tabId].redo
    )
    // Updates the scanningQueueDictionary
    if (
          ...
          async (response) => {
            if (chrome.runtime.lastError) {
              if (
                ...
                if (setting.A11yFix) {
                  // This one will continue with highlight later there is another highlight check
                  console.log('A11YFIXES_Start')
                  chrome.tabs.sendMessage(tabId, {
                    type: 'A11YFIXES_Start',
                    missingXpaths: data.framesDict,
                    tabId: tabId,
                  })
            ...
            if (response && response.success) {
              ...
              } else if (settings[tabId].highlight) {
                // Just highlight
                console.log('MISSING_FOUND HIGHLIGHT')
                chrome.tabs.sendMessage(tabId, {
                  type: 'HIGHLIGHT',
    ...
    } else if (scanningQueueDictionary[tabId].redo === true) {
      // there is a new scan coming in so need to rescan again (Could be from a mutation or something)
      console.log('There was a another request of scanning during a scan')
      chrome.tabs.sendMessage(tabId, { type: 'START_RESCANNING', tabId: tabId })
    }
    ...
    if (settings[request.tabId].A11yFix) {
      chrome.tabs.sendMessage(tabId, {
        type: 'A11YFIXES_Start',
        missingXpaths: missingXpaths.framesDict,
        tabId: tabId,
      })
    } else {
    ...
    Promise.all(fetchPromises).then(() => {
      ...
      // once we got all the frames we can get send in it to fix the label
      chrome.tabs.sendMessage(tabId, {
        type: 'SET_ARIA_LABELS',
        missingXpaths: arialLabelsFramesDict,
      })
    })
  }
...
async function addAttributeEventList(tabId, domDictionary, eventListnersList) {
  const promises = Object.keys(eventListnersList).map(
    async (backendDOMNodeId) => {
      try {
        ...
        if (!correspondingNodeId) {
          console.error(
            `No corresponding node found for backendDOMNodeId: ${backendDOMNodeId}`
          )
        ...
        console.error(
          `Failed to set attribute for backendDOMNodeId: ${backendDOMNodeId}`,
          error
        )
      }
    }
...
async function clearDataForTab(tabId) {
  ...
  chrome.storage.local.get(null, (items) => {
    if (chrome.runtime.lastError) {
      console.error(
        `Error retrieving storage items: ${chrome.runtime.lastError}`
      )
      return
    }
    ...
    if (keysToRemove.length > 0) {
      chrome.storage.local.remove(keysToRemove, () => {
        if (chrome.runtime.lastError) {
          console.error(
            `Error removing data for tab ${tabId}: ${chrome.runtime.lastError}`
          )
        } else {
  ...
  return new Promise((resolve, reject) => {
    chrome.tabs.sendMessage(tabId, { type: messageType }, (response) => {
      ...
      if (chrome.runtime.lastError) {
        ...
        reject(
          new Error(
            `Unexpected response status. Expected: ${expectedStatus}, but got: ${response ? response.status : 'no response'}`
          )
        )
      }
    })
    ...
    setTimeout(() => {
      reject(
        new Error(`Timeout waiting for ${expectedStatus} from tab ${tabId}`)
      )
        ...
        chrome.scripting.executeScript(
          ...
          () => {
            if (chrome.runtime.lastError) {
              reject(
                new Error(
                  `Failed to inject script ${scriptName} into tab ${tabId}: ${chrome.runtime.lastError.message}`
                )
              )
            } else {
  ...
  for (let key in previousSCANINFODICT) {
    if (mergedFramesDict[key]) {
      ...
      previousSCANINFODICT[key].forEach((previousItem) => {
        ...
        console.log(
          `Added missing xpath: ${previousItem.xpath} from previous scan info`
        )
      })
