...
const callback = async (mutationList, observer) => {
  ...
  for (const mutation of mutationList) {
    if (mutation.type === 'childList') {
      ...
      if (
        (addedNodesResult.includes(true) ||
          removedNodesResult.includes(true)) &&
        isVisibleFocusAble(mutation.previousSibling)
      ) {
...
const startObserving = () => {
  ...
  timeout = setTimeout(() => {
    if (!hasMutations) {
      isStable = true // Mark the DOM as stable
      console.log(
        'No mutations detected after DOMContentLoaded, setting DOM as stable.'
      )
      if (isConnectedDebuuger) {
