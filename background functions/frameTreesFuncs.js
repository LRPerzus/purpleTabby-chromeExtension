...
export async function settingAttributeNode(
  tabId,
  backendDOMNodeIds,
  domDictionary
) {
  ...
  const promises = Object.keys(backendDOMNodeIds).map(async (backendId) => {
    try {
      ...
      const attribute = await setAttributeValue(
        tabId,
        correspondingNodeId,
      ...
      if (attribute !== true && attribute === 'redo') {
        ...
        if (
          tryAgain.nodeName !== '::before' &&
          tryAgain.nodeName !== '::after' &&
          correspondingNodeId !== tryAgain.nodeId
        ) {
